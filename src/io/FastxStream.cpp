...
FastqDataPairChunk *FastqFileReader::readNextPairChunk() {
  ...
  if (r > 0) {
    if (r == toRead) {
      chunkEnd =
        cbufSize - SwapBufferSize; // SwapBuffersize defined in FastqStream.h as constant value : 1<<13;
  ...
  if (r > 0) {
    if (r == toRead) {
      chunkEnd_right =
        cbufSize_right - SwapBufferSize; // SwapBuffersize defined in FastqStream.h as constant value : 1<<13;
  ...
  if (!eof) {
    ...
    //  std::cout << "!!!! " << chunkEnd_right-cbufSize <<" diff:"<<difference << std::endl;
